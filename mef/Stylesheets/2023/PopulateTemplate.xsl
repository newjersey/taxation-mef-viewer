...
-->
	<xsl:template name="PopulatePercent">
		...
		<span>
			...
			<xsl:call-template name="PopulateChgHistory">
				<xsl:with-param name="targetNode" select="$TargetNode"/>
				<xsl:with-param name="usePercent" select="1"/>
				<!-- Jessica: Flag indicates % process needed -->
			</xsl:call-template>
		</span>
	</xsl:template>
	<!--
...
-->
	<xsl:template name="FormatPercent">
		<xsl:param name="s0"/>
		<xsl:choose>
			<xsl:when test="number($s0)=0">
				<!-- If the percent value is 0, display 0 -->
				...
				<xsl:choose>
					<xsl:when test="contains($s0,'.')">
						<xsl:variable name="s1">
							<xsl:choose>
								<xsl:when test="number(substring-before($s0,'.'))=0">
									<!-- If digits exist at the left of decimal, s1 is set to empty -->
									<xsl:value-of select="''"/>
								</xsl:when>
								<xsl:otherwise>
									<!-- If there is no digit at the left of decimal, set s1 to the data value-->
						...
						<xsl:variable name="s5" select="number(concat($s1,$s3))"/>
						<!-- combine digits at left of decimal with the first two digits at the right of decimal-->
						<xsl:choose>
							<xsl:when test="string-length($s2) &gt; 2">
								<!-- If more than 2 digits are at the right of decimal, move decimal 2 places to the right of the data value  -->
								<xsl:value-of select="concat(string($s5),'.',$s4)"/>
							</xsl:when>
							<xsl:otherwise>
								<xsl:choose>
									<xsl:when test="string-length($s2)=1">
										<!-- If only 1 digit is at the right of decimal, add 0 to the end -->
										<xsl:value-of select="concat(string($s5),'0')"/>
									</xsl:when>
									<xsl:otherwise>
										<!-- If there are 2 digits at the right of decimal, there is nothing to append-->
					...
					<xsl:otherwise>
						<xsl:value-of select="concat($s0,'00')"/>
						<!-- If there is no decimal in data, add 2 zeros  -->
					</xsl:otherwise>
				</xsl:choose>
			</xsl:otherwise>
		</xsl:choose>
...
-->
	<xsl:template name="PopulateCheckbox">
		...
		<xsl:attribute name="TabIndex"><xsl:value-of select="$TabOrder"/></xsl:attribute>
		<xsl:attribute name="id"><xsl:choose><xsl:when test="$IDVal = '/..'"><!-- If the data element does not exist, set id to BackupName --><xsl:value-of select="$BackupName"/></xsl:when><xsl:otherwise><!-- If the data element does exist, set id to IDVal --><xsl:value-of select="$IDVal"/></xsl:otherwise></xsl:choose></xsl:attribute>
		<xsl:attribute name="onclick"><xsl:value-of select="$CheckboxDisable"/></xsl:attribute>
		<xsl:call-template name="PopulateChgHistory">
			<xsl:with-param name="targetNode" select="$TargetNode"/>
			<xsl:with-param name="chxBoxType" select="'notBoolean'"/>
			<!-- Jessica: Flag indicates Check Box process needed -->
		</xsl:call-template>
	</xsl:template>
	<!--
***************************************************************************************************************************************************************
...
-->
	<xsl:template name="PopulateEnumeratedCheckbox">
		...
		<!-- This is the checkbox that is currently being displayed, need to compare this value with the TargetNode value-->
		<xsl:param name="TabOrder">-1</xsl:param>
		<xsl:param name="CheckboxDisable">return false;</xsl:param>
		<xsl:param name="BackupName"/>
		<xsl:param name="IDVal">
		...
		<xsl:attribute name="onclick"><xsl:value-of select="$CheckboxDisable"/></xsl:attribute>
		<xsl:attribute name="id"><xsl:choose><xsl:when test="$IDVal = '/..'"><xsl:value-of select="$BackupName"/></xsl:when><!-- If the data element does not exist, set id to BackupName --><xsl:otherwise><xsl:value-of select="$IDVal"/><xsl:value-of select="$DisplayedCheckboxValue"/></xsl:otherwise><!-- If the data element exists, set id to IDVal concatenated with the DisplayedCheckboxValue value for a unique identifier--></xsl:choose></xsl:attribute>
		<xsl:attribute name="TabIndex"><xsl:value-of select="$TabOrder"/></xsl:attribute>
		<xsl:call-template name="PopulateChgHistory">
			<xsl:with-param name="targetNode" select="$TargetNode"/>
			<xsl:with-param name="chxBoxType" select="'Enumerated'"/>
			<!-- Flag indicates that checkbox uses schema-supplied enumerated values to determine checkbox status-->
...
-->
	<xsl:template name="PopulateYesCheckbox">
		...
		<!-- If checkbox type is ennumerated w/ a 1 or 2 instead of 0 or 1 -->
		<xsl:attribute name="onclick"><xsl:value-of select="$CheckboxDisable"/></xsl:attribute>
		<xsl:attribute name="id"><xsl:choose><xsl:when test="$IDVal = '/..'"><xsl:value-of select="$BackupName"/>Yes</xsl:when><!-- If the data element does not exist, set id to BackupName --><xsl:otherwise><xsl:value-of select="$IDVal"/>Yes</xsl:otherwise><!-- If the data element exists, set id to IDVal --></xsl:choose></xsl:attribute>
		<xsl:attribute name="TabIndex"><xsl:value-of select="$TabOrder"/></xsl:attribute>
		<xsl:call-template name="PopulateChgHistory">
			<xsl:with-param name="targetNode" select="$TargetNode"/>
			<xsl:with-param name="chxBoxType" select="'Yes'"/>
			<!-- Jessica: Flag indicates Yes Check Box process needed -->
...
-->
	<xsl:template name="PopulateNoCheckbox">
		...
		<xsl:param name="Checkbox1or2Type">no</xsl:param>
		<xsl:attribute name="onclick"><xsl:value-of select="$CheckboxDisable"/></xsl:attribute>
		<xsl:attribute name="id"><xsl:choose><xsl:when test="$IDVal = '/..'"><xsl:value-of select="$BackupName"/>No</xsl:when><!-- If the data element does not exist, set id to BackupName --><xsl:otherwise><xsl:value-of select="$IDVal"/>No</xsl:otherwise><!-- If the data element exists, set id to IDVal --></xsl:choose></xsl:attribute>
		<xsl:attribute name="TabIndex"><xsl:value-of select="$TabOrder"/></xsl:attribute>
		<xsl:call-template name="PopulateChgHistory">
			<xsl:with-param name="targetNode" select="$TargetNode"/>
			<xsl:with-param name="chxBoxType" select="'No'"/>
			<!-- Flag indicates No Check Box process needed -->
			<xsl:with-param name="Checkbox1or2Type" select="$Checkbox1or2Type"/>
		</xsl:call-template>
	</xsl:template>
	<!--
...
-->
	<xsl:template name="PopulateYesBox">
		...
		<span>
			...
			<xsl:call-template name="PopulateChgHistory">
				<xsl:with-param name="targetNode" select="$TargetNode"/>
				<xsl:with-param name="txtBoxType" select="'Yes'"/>
				<!-- Flag indicates Yes Box with 'X' mark process needed -->
			</xsl:call-template>
		</span>
	</xsl:template>
	<!--
...
-->
	<xsl:template name="PopulateNoBox">
		...
		<span>
			...
			<xsl:call-template name="PopulateChgHistory">
				<xsl:with-param name="targetNode" select="$TargetNode"/>
				<xsl:with-param name="txtBoxType" select="'No'"/>
				<!-- Flag indicates Yes Box with 'X' mark process needed -->
			</xsl:call-template>
		</span>
	</xsl:template>
...
-->
	<xsl:template name="PopulateYesBoxText">
		...
		<span>
			...
			<xsl:call-template name="PopulateChgHistory">
				<xsl:with-param name="targetNode" select="$TargetNode"/>
				<xsl:with-param name="txtBoxType" select="'YesTxt'"/>
				<!-- Flag indicates Yes Box with 'Yes' mark process needed -->
			</xsl:call-template>
		</span>
	</xsl:template>
	<!--
...
-->
	<xsl:template name="PopulateNoBoxText">
		...
		<span>
			...
			<xsl:call-template name="PopulateChgHistory">
				<xsl:with-param name="targetNode" select="$TargetNode"/>
				<xsl:with-param name="txtBoxType" select="'NoTxt'"/>
				<!-- Flag indicates Yes Box with 'Yes' mark process needed -->
			</xsl:call-template>
		</span>
	</xsl:template>
...
-->
	<xsl:template name="PopulateLabel">
		...
		<xsl:attribute name="for"><xsl:choose><xsl:when test="$ForVal = '/..'"><xsl:value-of select="$BackupName"/></xsl:when><xsl:when test="$DisplayedCheckboxValue"><xsl:value-of select="$ForVal"/><xsl:value-of select="$DisplayedCheckboxValue"/></xsl:when><xsl:otherwise><xsl:value-of select="$ForVal"/></xsl:otherwise></xsl:choose></xsl:attribute>
	</xsl:template>
	<!--
***************************************************************************************************************************************************************
...
-->
	<xsl:template name="PopulateLabelYes">
		...
		<xsl:attribute name="for"><xsl:choose><xsl:when test="$ForVal = '/..'"><xsl:value-of select="$BackupName"/>Yes</xsl:when><xsl:otherwise><xsl:value-of select="$ForVal"/>Yes</xsl:otherwise></xsl:choose></xsl:attribute>
	</xsl:template>
	<!--
***************************************************************************************************************************************************************
...
-->
	<xsl:template name="PopulateLabelNo">
		...
		<xsl:attribute name="for"><xsl:choose><xsl:when test="$ForVal = '/..'"><xsl:value-of select="$BackupName"/>No</xsl:when><xsl:otherwise><xsl:value-of select="$ForVal"/>No</xsl:otherwise></xsl:choose></xsl:attribute>
	</xsl:template>
	<!--
***************************************************************************************************************************************************************
Name:           PopulateMonth
	...
	<xsl:template name="PopulateMonth">
		...
		<span>
			...
			<xsl:call-template name="PopulateChgHistory">
				...
				<!-- Jessica: 'start,length' as in substring(string,start,length) -->
			</xsl:call-template>
		</span>
	</xsl:template>
	<!--
...
-->
	<xsl:template name="PopulateDay">
		...
		<span>
			...
			<xsl:call-template name="PopulateChgHistory">
				<xsl:with-param name="targetNode" select="$TargetNode"/>
				<xsl:with-param name="subString" select="'9,2'"/>
				<!-- Jessica: 'start,length' as in substring(string,start,length) -->
			</xsl:call-template>
		</span>
	</xsl:template>
...
-->
	<xsl:template name="PopulateYear">
		...
		<span>
			...
			<xsl:call-template name="PopulateChgHistory">
				<xsl:with-param name="targetNode" select="$TargetNode"/>
				<xsl:with-param name="subString" select="'1,4'"/>
				<!-- Jessica: 'start,length' as in substring(string,start,length) -->
			</xsl:call-template>
		</span>
	</xsl:template>
...
-->
	<xsl:template name="PopulateMonthForMonthDayType">
		...
		<span>
			...
			<xsl:call-template name="PopulateChgHistory">
				<xsl:with-param name="targetNode" select="$TargetNode"/>
				<xsl:with-param name="subString" select="'3,2'"/>
				<!-- Jessica: 'start,length' for substring(string,start,length) -->
			</xsl:call-template>
		</span>
	</xsl:template>
	<!--
	...
	<xsl:template name="PopulateMonthYearForYearMonthType">
		...
		<span>
			...
			<xsl:call-template name="PopulateChgHistory">
				<xsl:with-param name="targetNode" select="$TargetNode"/>
				<xsl:with-param name="subString" select="'6,2'"/>
				<!-- 'start,length' for substring(string,start,length) -->
			</xsl:call-template> -    
      <xsl:call-template name="PopulateChgHistory">
				<xsl:with-param name="targetNode" select="$TargetNode"/>
				<xsl:with-param name="subString" select="'1,4'"/>
	...
	<xsl:template name="PopulateCityStateInfo">
		...
		<xsl:choose>
			<xsl:when test="$TargetNode">
				...
				<xsl:if test="($TargetNode/CityNm != '') and ($TargetNode/StateAbbreviationCd != '')">, </xsl:if>
				<xsl:call-template name="PopulateText">
					<xsl:with-param name="TargetNode" select="$TargetNode/StateAbbreviationCd"/>
				</xsl:call-template>
				<span style="width:7px;"/>
			...
			<xsl:otherwise>
				<xsl:if test="$ForeignAddressTargetNode/CityNm">
					...
					<xsl:if test="($ForeignAddressTargetNode/CityNm != '') and ($ForeignAddressTargetNode/ProvinceOrStateNm != '')">, </xsl:if>
					<xsl:if test="($ForeignAddressTargetNode/CityNm != '') and not($ForeignAddressTargetNode/ProvinceOrStateNm)">
						<span style="width:7px;"/>
					</xsl:if>
				</xsl:if>
	...
	<xsl:template name="PopulateReturnHeaderFiler">
		...
		<xsl:choose>
			<xsl:when test="$Location='PAR'">
				<!-- Indicating that the data needs to be pulled from Parent return header-->
				<xsl:choose>
					...
					<xsl:otherwise>
						<!-- Handles all other data. Populate data whose element name matches TargetNode element name-->
						<xsl:call-template name="PopulateText">
			...
			<xsl:when test="$Location='SUB'">
				<!-- Indicating that data needs to be pulled from Subsidiary return header-->
			...
			<xsl:otherwise>
				<!-- Indicating that the data needs to be pulled from Filer return header-->
				<xsl:choose>
					...
					<xsl:when test="$TargetNode='BusinessNameLine2Txt' and ($RtnHdrData/Filer/TradeName/BusinessNameLine2Txt or $RtnHdrData/Filer/TrusteeBusinessName/BusinessNameLine2Txt)">
						<!-- Catch for 94X and 5227 forms, which can have two different elements of type BusinessName -->
						<xsl:call-template name="PopulateText">
					...
					<xsl:when test="$TargetNode = 'EIN' ">
						...
						<xsl:if test="$TaxpayerPrint = 'false' or $TaxpayerPrint = '0'">
							<xsl:if test="($EINChanged='true' or $EINChanged='1') and ((/AppData/Parameters/TINChanged='true') or (/AppData/Parameters/TINChanged='1'))">
								...
								<img src="{$NonVersionedImagePath}/changeSmall.gif" style="padding-top:1px;" alt="EIN has changed" title="EIN has changed"/>
							</xsl:if>
						</xsl:if>
					</xsl:when>
					<xsl:when test="$TargetNode = 'Phone' ">
...
-->
	<xsl:template name="PopulateReturnHeaderTaxPeriodBeginDate">
		<xsl:choose>
			<xsl:when test="$Location='PAR'">
				<!-- Indicating the element needs to be pulled Parent return header -->
				<xsl:call-template name="PopulateMonthDayYear">
					<xsl:with-param name="TargetNode" select="$ParRtnHdrData/TaxPeriodBeginDt"/>
				</xsl:call-template>
			</xsl:when>
			<xsl:when test="$Location='SUB'">
				<!-- Indicating the element needs to be pulled Subsidiary return header-->
				<xsl:call-template name="PopulateMonthDayYear">
					<xsl:with-param name="TargetNode" select="$SubRtnHdrData/TaxPeriodBeginDt"/>
				</xsl:call-template>
			</xsl:when>
			<xsl:when test="$Location='ELM'">
				<!-- Indicating the element needs to be pulled Subsidiary return header-->
				<xsl:call-template name="PopulateMonthDayYear">
					<xsl:with-param name="TargetNode" select="$SubRtnHdrData/TaxPeriodBeginDt"/>
				</xsl:call-template>
			</xsl:when>
			<xsl:when test="$Location='ADJ'">
				<!-- Indicating the element needs to be pulled Subsidiary return header-->
				<xsl:call-template name="PopulateMonthDayYear">
					<xsl:with-param name="TargetNode" select="$SubRtnHdrData/TaxPeriodBeginDt"/>
				</xsl:call-template>
			</xsl:when>
			<xsl:when test="$Location='SSC'">
				<!-- Indicating the element needs to be pulled Subsidiary return header-->
				<xsl:call-template name="PopulateMonthDayYear">
					<xsl:with-param name="TargetNode" select="$SubRtnHdrData/TaxPeriodBeginDt"/>
				</xsl:call-template>
			</xsl:when>
			<xsl:when test="$Location='SEL'">
				<!-- Indicating the element needs to be pulled Subsidiary return header-->
				<xsl:call-template name="PopulateMonthDayYear">
					<xsl:with-param name="TargetNode" select="$SubRtnHdrData/TaxPeriodBeginDt"/>
				</xsl:call-template>
			</xsl:when>
			<xsl:when test="$Location='SAJ'">
				<!-- Indicating the element needs to be pulled Subsidiary return header-->
				<xsl:call-template name="PopulateMonthDayYear">
					<xsl:with-param name="TargetNode" select="$SubRtnHdrData/TaxPeriodBeginDt"/>
				</xsl:call-template>
			</xsl:when>
			<xsl:when test="$Location='LSC'">
				<!-- Indicating the element needs to be pulled Subsidiary return header-->
				<xsl:call-template name="PopulateMonthDayYear">
					<xsl:with-param name="TargetNode" select="$SubRtnHdrData/TaxPeriodBeginDt"/>
				</xsl:call-template>
			...
			<xsl:when test="$Location='LEL'">
				<!-- Indicating the element needs to be pulled Subsidiary return header-->
				<xsl:call-template name="PopulateMonthDayYear">
					<xsl:with-param name="TargetNode" select="$SubRtnHdrData/TaxPeriodBeginDt"/>
				</xsl:call-template>
			</xsl:when>
			<xsl:when test="$Location='LAJ'">
				<!-- Indicating the element needs to be pulled Subsidiary return header-->
				<xsl:call-template name="PopulateMonthDayYear">
					<xsl:with-param name="TargetNode" select="$SubRtnHdrData/TaxPeriodBeginDt"/>
				</xsl:call-template>
			</xsl:when>
			<xsl:when test="$Location='PSC'">
				<!-- Indicating the element needs to be pulled Subsidiary return header-->
				<xsl:call-template name="PopulateMonthDayYear">
					<xsl:with-param name="TargetNode" select="$SubRtnHdrData/TaxPeriodBeginDt"/>
				</xsl:call-template>
			...
			<xsl:otherwise>
				<!-- Indicating it needs to pull from filer info -->
				<xsl:call-template name="PopulateMonthDayYear">
...
-->
	<xsl:template name="PopulateReturnHeaderTaxYear">
		<xsl:if test="$Location='RET'">
			<!-- Indicating the element needs to be pulled from consolidated return header-->
			<xsl:call-template name="PopulateText">
...
-->
	<xsl:template name="FormTitle">
		...
		<xsl:param name="TY" select="'TY '"/>
		<xsl:param name="Form" select="' Form '"/>
		<xsl:param name="Schedule" select="' Schedule '"/>
		<xsl:param name="AMT" select="' AMT'"/>
		<xsl:param name="POL" select="'-POL'"/>
		<xsl:param name="K1" select="'K-1'"/>
		<xsl:param name="Year" select="substring($Version,1,4)"/>
		<xsl:param name="Name" select="substring-after($RootElement, 'IRS')"/>
		<xsl:param name="BeforeSchedule" select="substring-before($Name, 'Schedule')"/>
		<xsl:param name="AfterSchedule" select="substring-after($Name, 'Schedule')"/>
		<xsl:param name="BeforeAMT" select="substring-before($Name, 'AMT')"/>
		<xsl:param name="BeforePOL" select="substring-before($Name, 'POL')"/>
		<xsl:choose>
			<xsl:when test="contains($Name, 'Schedule')">
				<xsl:choose>
					<xsl:when test="$AfterSchedule = 'K1'">
						<xsl:value-of select="concat($TY, $Year, $Form, $BeforeSchedule, $Schedule, $K1)"/>
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of select="concat($TY, $Year, $Form, $BeforeSchedule, $Schedule, $AfterSchedule)"/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:when>
			<xsl:when test="contains($Name, 'AMT')">
				<xsl:value-of select="concat($TY, $Year, $Form, $BeforeAMT, $AMT)"/>
			</xsl:when>
			<xsl:when test="contains($Name, 'POL')">
				<xsl:value-of select="concat($TY, $Year, $Form, $BeforePOL, $POL)"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="concat($TY, $Year, $Form, $Name)"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	<!--
...
-->
    ...
    <script>
      // In Edge, wait until the whole document is loaded before executing these functions for highlighting fields from clicking on the ack.
      window.addEventListener('DOMContentLoaded', function() {   
        // scroll to the field pointed to in XPath.
        var xpath = '<xsl:value-of select="$XPath"/>';
        
        if (xpath != '' &amp;&amp; rtnTree.ackErrorHiLited == '') { //Jessica: see associated code in SubmissionTreeBase.xsl
...
-->
	<xsl:template name="SetTableToggleButton">
		...
		<xsl:param name="imageID">
			<xsl:value-of select="$containerID"/>image</xsl:param>
		<xsl:param name="buttonID">
			<xsl:value-of select="$containerID"/>button</xsl:param>
...
-->
	<xsl:template name="SetDynamicTableToggleButton">
		...
		<xsl:param name="buttonID">
			<xsl:value-of select="$containerID"/>button</xsl:param>
		<xsl:param name="headerRowCount">
			<xsl:value-of select="$headerHeight"/>
		</xsl:param>
		<xsl:param name="displayRowCount">
...
-->
	<xsl:template name="SetFormLinkInlineRRD">
		...
		<xsl:param name="HeaderColor">
			<xsl:value-of select="$UserPref/View/TableHeaderBgColor"/>
		</xsl:param>
		<xsl:param name="OddRowColor">
			<xsl:value-of select="$UserPref/View/TableRow1BgColor"/>
		</xsl:param>
		<xsl:param name="EvenRowColor">
			<xsl:value-of select="$UserPref/View/TableRow2BgColor"/>
		</xsl:param>
			...
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:variable name="TextVal">
						<xsl:choose>
							<xsl:when test="name($TargetNode/self::*) = name($TargetNode)">
	...
	<xsl:template name="PopulateCommonLeftover">
		...
		<tr>
			...
			<td class="styLeftOverTableRowAmount" style="width:{$AmountWidth}mm;">
				<xsl:call-template name="PopulateText">
					<xsl:with-param name="TargetNode" select="$TargetNode/@softwareId"/>
				</xsl:call-template>
			</td>
		</tr>
		<tr>
			...
			<td class="styLeftOverTableRowAmount" style="width:{$AmountWidth}mm;">
				<xsl:call-template name="PopulateText">
					<xsl:with-param name="TargetNode" select="$TargetNode/@softwareVersionNum"/>
				</xsl:call-template>
			</td>
		</tr>
		<xsl:if test="$Print = $Separated">
			...
			<xsl:choose>
				<xsl:when test="$PrimarySSNExists != ''">
					...
					<xsl:if test="$SpouseSSNExists != ''">
						<!-- Indicating that the Spouse SSN should be displayed -->
						<tr>
...
-->
	<xsl:template name="PopulateDepCommonLeftoverNameEIN">
		...
		<xsl:choose>
			<xsl:when test="$Location='PAR'">
				<!-- Indicating that data should be pulled from parent return header -->
			...
			<xsl:when test="$Location='SUB'">
				<!-- Indicating that data should be pulled from subsidiary return header-->
			...
			<xsl:otherwise>
				<!-- Indicating that data should be pulled from filer return header -->
...
-->
	<xsl:template name="PopulateDepCommonLeftoverLandscape">
		<xsl:param name="TargetNode"/>
		<xsl:call-template name="PopulateDepCommonLeftoverNameEINLandscape"/>
		<xsl:if test="(($TargetNode/@softwareId) or ($TargetNode/@softwareId != ''))">
...
-->
	<xsl:template name="PopulateDepCommonLeftoverNameEINLandscape">
		...
		<xsl:choose>
			<xsl:when test="$Location='PAR'">
				<!-- Indicating that data should be pull from parent return header -->
				<xsl:if test="$ParRtnHdrData/ParentCorpGrp/BusinessName/BusinessNameLine2Txt and $ParRtnHdrData/ParentCorpGrp/BusinessName/BusinessNameLine2Txt != '' ">
					<div class="styTopSectionLineLandscape">
						<div class="styTopSectionLineLbl" style="float:left;clear:none;">&#160;</div>
						<div style="width:700px;font-size:10pt;float:left;clear:none;">
			...
			<xsl:when test="$Location='SUB'">
				<!-- Indicating that data should be pull from subsidiary return header-->
				<xsl:if test="$SubRtnHdrData/SubsidiaryCorpGrp/BusinessName/BusinessNameLine2Txt and $SubRtnHdrData/SubsidiaryCorpGrp/BusinessName/BusinessNameLine2Txt != '' ">
					<div class="styTopSectionLineLandscape">
						<div class="styTopSectionLineLbl" style="float:left;clear:none;">&#160;</div>
						<div style="width:700px;font-size:10pt;float:left;clear:none;">
			...
			<xsl:otherwise>
				<!-- Indicating that data should be pull from filer return header -->
				<xsl:if test="$RtnHdrData/Filer/BusinessName/BusinessNameLine2Txt and $RtnHdrData/Filer/BusinessName/BusinessNameLine2Txt != '' ">
					<div class="styTopSectionLineLandscape">
						<div class="styTopSectionLineLbl" style="float:left;clear:none;">&#160;</div>
						<div style="width:700px;font-size:10pt;float:left;clear:none;">
	...
	<xsl:template name="PopulateDisplayName">
		...
		<xsl:choose>
			<xsl:when test="$DisplayName != ''">
				<xsl:value-of select="concat('TY ', $Year, ' ', $DisplayName)"/>
			</xsl:when>
			<xsl:otherwise>
				<xsl:value-of select="concat('TY ', $Year, ' ', local-name($TargetNode))"/>
			</xsl:otherwise>
		</xsl:choose>
	</xsl:template>
	<!--
...
-->
	<xsl:template name="PopulateChgHistory">
		...
		<xsl:choose>
			<xsl:when test="name($targetNode/self::*) = name($targetNode)">
				<!-- targetNode points to a NODE -->
				<xsl:call-template name="displayData">
					<xsl:with-param name="targetNode" select="$targetNode"/>
					<xsl:with-param name="latestNode" select="$targetNode/ChangeHistory"/>
					<!-- Possible matched NODE under Change History tag -->
			...
			<xsl:otherwise>
				<!-- targetNode points to an ATTRIBUTE -->
				<xsl:call-template name="displayData">
					<xsl:with-param name="targetNode" select="$targetNode"/>
					<xsl:with-param name="latestNode" select="$targetNode/parent::*/ChangeHistory/@*[name() = name($targetNode)]"/>
					<!-- Jessica: Possible matched ATTRIBUTE under Change History tag -->
					<xsl:with-param name="usePercent" select="$usePercent"/>
					<xsl:with-param name="subString" select="$subString"/>
					<xsl:with-param name="chxBoxType" select="$chxBoxType"/>
					<xsl:with-param name="Checkbox1or2Type" select="$Checkbox1or2Type"/>
	...
	<xsl:template name="displayData">
		...
		<xsl:variable name="argVal">
			...
			<xsl:choose>
				<xsl:when test="not($latestNode) or $latestNode='' or not(name($targetNode/self::*) = name($targetNode))">
					<xsl:choose>
						<xsl:when test="$Checkbox1or2Type='yes'">
							<!-- If checkbox type is ennumerated 1 or 2, 
                                                              subtract 1 from targetNode                             
                                                              and use that value as argVal-->
							<xsl:value-of select="($targetNode)-1"/>
						</xsl:when>
						<xsl:otherwise>
							<!-- otherwise, use existing targetNode value -->
							<xsl:value-of select="$targetNode"/>
						</xsl:otherwise>
					</xsl:choose>
				</xsl:when>
		...
		<xsl:choose>
			...
			<xsl:when test="not(not($latestNode) or $latestNode='') and ($thisValue != $latestValue)">
				<xsl:choose>
					<xsl:when test="$chxBoxType">
						<!-- For checkbox type -->
						<xsl:if test="($TaxpayerPrint = 'false' or $TaxpayerPrint = '0')">
							<!-- If parameter TaxpayerPrint is false or '0', then set these attributes-->
                    ...
                    outline:2px solid <xsl:value-of select="$UserPref/View/ChangedFieldBgColor"/></xsl:attribute>
								<xsl:attribute name="title"><xsl:choose><xsl:when test="$Stage='original'">Latest: <xsl:choose><xsl:when test="$latestChxBoxFlag = 'chxed'">Checked</xsl:when><xsl:otherwise>Unchecked</xsl:otherwise></xsl:choose></xsl:when><xsl:otherwise>Original: <xsl:choose><xsl:when test="$thisChxBoxFlag = 'chxed'">Checked</xsl:when><xsl:otherwise>Unchecked</xsl:otherwise></xsl:choose></xsl:otherwise></xsl:choose></xsl:attribute>
							</xsl:if>
						</xsl:if>
						<xsl:choose>
							<xsl:when test="$Stage='original'">
								<!-- Check for these conditions: data is checkbox type, Stage is original, and thisChxBoxFlag is true. If so, set attribute thisChxBoxFlag to checked-->
								<xsl:if test="$thisChxBoxFlag = 'chxed'">
									<xsl:attribute name="checked">checked</xsl:attribute>
								</xsl:if>
							</xsl:when>
							<xsl:otherwise>
								<!-- Check for these conditions: data is checkbox type, Stage is latest, latestChxBoxFlag is true. If so, set attribute latestChxBoxFlag to checked-->
					...
					<xsl:otherwise>
						<!-- For non-checkbox type -->
						<xsl:if test="$TaxpayerPrint = 'false' or $TaxpayerPrint = '0'">
							<!-- If parameter TaxpayerPrint is false or '0', then set these attributes-->
							<xsl:attribute name="style">                      
                  background-color: <xsl:value-of select="$UserPref/View/ChangedFieldBgColor"/><xsl:choose><xsl:when test="((not(string($thisValue)) or normalize-space($thisValue)='&nbsp;') and $Stage='original') or ((not(string($latestValue)) or normalize-space($latestValue)='&nbsp;') and $Stage='latest')">              
                    ; width:35%
                  </xsl:when><xsl:otherwise>; display:inline;</xsl:otherwise></xsl:choose></xsl:attribute>
							<xsl:attribute name="title"><xsl:choose><xsl:when test="$Stage='original'">Latest: <xsl:value-of select="$latestValue"/></xsl:when><xsl:otherwise>Original: <xsl:value-of select="$thisValue"/></xsl:otherwise></xsl:choose></xsl:attribute>
						</xsl:if>
						<xsl:choose>
							<!-- Check for these conditions: data is not checkbox type and Stage is original. If so, display original data value-->
							<xsl:when test="$Stage='original'">
								<xsl:value-of select="$thisValue"/>
							</xsl:when>
							<xsl:otherwise>
								<!-- Check for these conditions: data is not checkbox type and Stage is latest. If so, display latest data value-->
			...
			<xsl:otherwise>
				<xsl:choose>
					<xsl:when test="$chxBoxType">
						<!-- If the data is checkbox type and thisCHxBoxFlag is set to checked, set attribute "checked" to checked  -->
						<xsl:if test="$thisChxBoxFlag = 'chxed'">
							<xsl:attribute name="checked">checked</xsl:attribute>
						</xsl:if>
					</xsl:when>
					<xsl:otherwise>
						<!-- If the data is not checkbox type, then display original data value-->
						<xsl:value-of select="$thisValue"/>
					</xsl:otherwise>
				</xsl:choose>
			</xsl:otherwise>
