...
<xsl:otherwise ><xsl:value-of select="$falseString" /></xsl:otherwise>
  ...
  <xsl:template name="insertScriptElement" >
    ...
    <script type="text/javascript" language="javascript">
      ...
      temp =   "<xsl:if test="($ackPresent = $trueString)">Acknowledgement<xsl:if test="($codeEditPresent = $trueString) or  ($procStatusPresent = $trueString) or ($changeHistoryPresent = $trueString) or ($indexPresent = $trueString)">,</xsl:if>
        </xsl:if>
        <xsl:if test="($codeEditPresent = $trueString)">CodeAndEdit<xsl:if test="($procStatusPresent = $trueString) or
($changeHistoryPresent = $trueString) or ($indexPresent = $trueString)">,</xsl:if>
        </xsl:if>
        <xsl:if test="($procStatusPresent = $trueString)">ProcessingStatus<xsl:if test="($changeHistoryPresent = $trueString) or ($indexPresent = $trueString)">,</xsl:if>
        </xsl:if>
        <xsl:if test="($changeHistoryPresent = $trueString)">ChangeHistory<xsl:if test="($indexPresent = $trueString)">,</xsl:if>
        </xsl:if>
...
function printDocsHelper()
{
  ...
  <xsl:template name="insertBodyElement" >
    ...
    <input type="hidden" name="filingStatus" value="{$ReturnStatus}" />
  ...
  <xsl:template name="displayAdditionalInfo" >
    ...
    <xsl:if test="(($ackPresent = $trueString) or($codeEditPresent = $trueString) or ($procStatusPresent= $trueString) or ($changeHistoryPresent = $trueString) or ($indexPresent = $trueString))" >
